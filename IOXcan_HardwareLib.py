...
def ThirdPartyId(DeviceId):
    ...
    message = can.Message(arbitration_id=0x000FDB81, data=[
                          DeviceId % 256, (DeviceId >> 8) % 256], is_extended_id=True)
    return message
...
def StatusData(statusId, value):
    ...
    message = can.Message(arbitration_id=0x000FDB80, data=[statusId % 256, (statusId >> 8) % 256, value % 256, (
        value >> 8) % 256, (value >> 16) % 256, (value >> 24) % 256], is_extended_id=True)
    return message
...
def PriorityStatusData(statusId, value):
    ...
    print("Status ID: " + str(statusId) + " (" + str(hex(statusId)) + ")")
    print("Value: " + str(value) + " (" + str(hex(value)) + ")" + "\n")
    message = can.Message(arbitration_id=0x000FDB87, data=[statusId % 256, (statusId >> 8) % 256, value % 256, (
        value >> 8) % 256, (value >> 16) % 256, (value >> 24) % 256], is_extended_id=True)
    return message
